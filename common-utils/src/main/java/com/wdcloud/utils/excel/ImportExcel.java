...
public class ImportExcel {
    ...
    /**
     * 构造函数,默认标题行号0，数据行号=标题行号+1
     *
     * @param fileName 导入文件，读取第一个工作表
     */
    public ImportExcel(String fileName) throws IOException {
    ...
    public ImportExcel(MultipartFile multipartFile) throws IOException {
        this(multipartFile.getInputStream(), 0, 0);
    }
    /**
     * 构造函数
     *
    ...
    public ImportExcel(MultipartFile multipartFile, int headerNum, int sheetIndex) throws IOException {
        this(multipartFile.getInputStream(), headerNum, sheetIndex);
    }
     ...
     * @param sheetIndex 工作表编号
     */
    public ImportExcel(InputStream is, int headerNum, int sheetIndex) throws IOException {
        if (!is.markSupported()) {
                    ...
                    try {
                        if (valType == String.class) {
                            if (val instanceof Number) {
                                val = new BigDecimal(new Double(val.toString())).toString();
                            } else {
